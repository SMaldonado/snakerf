...
v1.update_Vt(srf.V_fsk(v1.ts, fc, f_sym, f_dev, random_data, -50, n = 1))
v1.add_noise()
...
plt.subplot(2,1,1)
plt.plot(np.arange(min(v1.ts), max(v1.ts), 1/f_sample), srf.mag(y1))
plt.plot(np.arange(min(v1.ts), max(v1.ts), 1/f_sample), srf.mag(y2))
